<template>
  <div>
    ...
    <div class="search-bar">
      ...
      <input v-model="keyword" type="text" placeholder="标题、内容、地点" class="search-input" @keyup.enter="search()" />
    ...
    <table class="table">
      <thead>
      <tr>
        <th>ID</th><th>标题</th><th>区县</th><th>地点</th>
        <th>内容摘要</th>
        <th>报名开始</th><th>报名结束</th>
        <th>活动时间</th>
        <th>报名人数</th><th>操作</th>
      </tr>
      </thead>
      <tbody>
        <tr v-for="p in list" :key="p.id">
          ...
          <td>{{ p.region }}</td>
          <td>{{ p.location }}</td>
          <td class="cell-clip" :title="p.content">{{ truncate(p.content, 40) }}</td>
          <td>{{ formatTime(p.startTime) }}</td>
          <td>{{ formatTime(p.endTime) }}</td>
          <td>{{ formatTime(p.createTime) }}</td>
          <td>{{ p.registrationCount ?? 0 }}</td>
          <td>
    ...
    <div v-if="editing" class="modal" @click.self="editing = null">
      <div class="modal-box">
        ...
        <div class="form-item"><label>内容</label><textarea v-model="form.content" rows="4"></textarea></div>
        <div class="form-item"><label>地点</label><input v-model="form.location" /></div>
        <div class="form-item"><label>报名开始时间</label><input v-model="form.startTime" type="date" /></div>
        <div class="form-item"><label>报名结束时间</label><input v-model="form.endTime" type="date" /></div>
        <div class="form-item"><label>活动时间</label><input v-model="form.createTime" type="date" /></div>
...
import { adminGetActivities, adminCreateActivity, adminUpdateActivity, adminDeleteActivity, uploadImage } from '../../api'
...
async function load(p = 0) {
  const res = await adminGetActivities({ keyword: keyword.value || undefined, region: region.value || undefined, page: p, size: 10 })
  list.value = res.data || []
...
function showEdit(row) {
  if (row) {
    ...
    form.startTime = toDateOnly(row.startTime)
    form.endTime = toDateOnly(row.endTime)
    form.createTime = toDateOnly(row.createTime)
    form.coverImage = row.coverImage || ''
  } else {
    form.id = null
    form.title = form.region = form.content = form.location = form.coverImage = ''
    form.startTime = form.endTime = form.createTime = ''
  }
  editing.value = {}
}
...
async function save() {
  ...
  const payload = {
    ...
    startTime: form.startTime ? form.startTime + 'T00:00:00' : null,
    endTime: form.endTime ? form.endTime + 'T23:59:59' : null,
    createTime: form.createTime ? form.createTime + 'T00:00:00' : null,
  }
  try {
    if (form.id) {
      await adminUpdateActivity(form.id, payload)
    } else {
