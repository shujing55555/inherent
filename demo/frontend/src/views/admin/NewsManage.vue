<template>
  <div>
    ...
    <div class="search-bar">
      ...
      <input v-model="keyword" type="text" placeholder="标题、正文、来源" class="search-input" @keyup.enter="search()" />
    ...
    <table class="table">
      <thead>
        <tr><th>ID</th><th>标题</th><th>区县</th><th>来源</th><th>正文摘要</th><th>发布时间</th><th>操作</th></tr>
      </thead>
      <tbody>
        <tr v-for="p in list" :key="p.id">
          ...
          <td>{{ formatTime(p.publishTime) }}</td>
          <td>
    ...
    <div v-if="editing" class="modal" @click.self="editing = null">
      <div class="modal-box">
        ...
        <div class="form-item"><label>正文</label><textarea v-model="form.content" rows="6"></textarea></div>
        ...
        <div class="form-item"><label>来源</label><input v-model="form.source" /></div>
        <div class="form-item"><label>发布时间</label><input v-model="form.publishTime" type="date" /></div>
        <div class="actions">
...
import { adminGetNews, adminCreateNews, adminUpdateNews, adminDeleteNews, uploadImage } from '../../api'
...
const form = reactive({ id: null, title: '', region: '', content: '', coverImage: '', source: '', publishTime: '' })
...
async function load(p = 0) {
  const res = await adminGetNews({ keyword: keyword.value || undefined, region: region.value || undefined, page: p, size: 10 })
  list.value = res.data || []
...
function showEdit(row) {
  if (row) {
    ...
    form.publishTime = toDateOnly(row.publishTime)
  } else {
    form.id = null
    form.title = form.region = form.content = form.coverImage = form.source = ''
    form.publishTime = toDateOnly(new Date())
  }
  editing.value = {}
}
...
async function save() {
  ...
  const payload = { ...form }
  if (payload.publishTime) payload.publishTime = payload.publishTime + 'T00:00:00'
  try {
    if (form.id) {
