<template>
  <div class="stories-page container">
    ...
    <div v-else class="list">
      <div v-for="item in list" :key="item.id" class="card" @click="$router.push(`/stories/${item.id}`)">
        <img v-if="item.coverImage" :src="item.coverImage.startsWith('http') ? item.coverImage : baseUrl + item.coverImage" class="cover" alt="" />
        <div class="info">
          <h3>{{ item.title }}</h3>
          <p v-if="item.region" class="meta">{{ item.region }}</p>
          <p class="desc">{{ (item.content || '').replace(/<[^>]+>/g, '').slice(0, 120) }}...</p>
        </div>
      </div>
    </div>
...
async function load(p = 0) {
  ...
  loading.value = true
  try {
    const res = await getStoryList({ region: region.value || undefined, page: p, size: 10 })
    list.value = res.data || []
...
<style scoped>
.stories-page h2 { margin-bottom: 16px; }
...
.card .info .meta { font-size: 12px; color: #c41e3a; margin-bottom: 6px; }
